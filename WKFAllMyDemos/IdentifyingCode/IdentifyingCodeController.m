//
//  IdentifyingCodeController.m
//  WKFAllMyDemos
//
//  Created by 王孔飞 on 2017/7/28.
...
@interface IdentifyingCodeController ()<UITextFieldDelegate, UIAlertViewDelegate>
...
@implementation IdentifyingCodeController
...
- (void)viewDidLoad {
    ...
    //显示验证码界面
    _authCodeView = [[AuthcodeView alloc] initWithFrame:CGRectMake(30, 100, self.view.frame.size.width-60, 40)];
    [self.view addSubview:_authCodeView];
    
    NSLog(@"_authCodeView.authCodeStr == %@",_authCodeView.authCodeStr);
...
{
    //判断输入的是否为验证图片中显示的验证码
    if ([_input.text isEqualToString:_authCodeView.authCodeStr])
    {
        //正确弹出警告款提示正确
