...
+ (void)load {
    ...
    Method kf_imageNamed = class_getClassMethod(self, sel_getUid("kf_imageNamed"));
    
    // 交换两个方法的实现部分。
    // 即调用imageNamed就会执行hl_imageNamed的实现，调用hl_imageNamed就会执行imageNamed的实现。
    method_exchangeImplementations(imageNamed, kf_imageNamed);
    
}

// 一个带有扩展系统功能的方法实现。
+ (UIImage *)kf_imageNamed:(NSString *)name {
    // 此时会调用imageNamed的系统实现部分，因为实现部分已经做了交换。
    
    NSLog(@"kf_imageNamed name == %@",name);
    UIImage * image = [self kf_imageNamed:name];
    if (image) {
        NSLog(@"图片加载成功");
    } else {
        NSLog(@"图片加载失败");
