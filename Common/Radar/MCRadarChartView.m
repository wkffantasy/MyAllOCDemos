...
- (void)initialization {
    ...
    _radarLineWidth = 1.0;
    _radarStrokeColor = HEXRGBCOLOR(0x586e83);
    _radarFillColor = [UIColor clearColor];
...
- (void)drawRadarWithContext:(CGContextRef)context {
    ...
    for (NSInteger layer = 0; layer < _numberOfLayer; layer ++) {
      ...
      NSLog(@"layerRadius ==%f",layerRadius);
        CGContextAddArc(context,_centerPoint.x, _centerPoint.y, layerRadius, 0, 2*M_PI, 0);
...
- (CGPoint)pointAtIndex:(NSInteger)index radius:(CGFloat)radius {
  ...
  CGPoint point = CGPointMake(_centerPoint.x + radius * sin(index * angleOfValue), _centerPoint.y - radius * cos(index * angleOfValue));
  if (index==1) {
    self.secondPoint = point;
  }
  if (index==3 && self.secondPoint.y) {
    point.y = self.secondPoint.y;
  }
  return point;
